  ...
  <div class="dashboard-sidebar-suburbs">
    <template v-if="suburbs.length">
      ...
      <BaseButton
        ...
        :text="suburb.name"
        @click.native="$store.commit('dashboard/setSelectedSuburbIndex', index)"
      />
    </template>
    <BaseButton
      text="Add a suburb"
      @click.native="$store.commit('global/toggleSearchState')"
...
export default {
  ...
  computed: mapState({
    suburbs: state => state.dashboard.suburbs.map(suburb => suburb.details),
    selectedSuburbIndex: state => state.dashboard.selectedSuburbIndex
  })
}
</script>
