...
export default {
  ...
  mounted () {
    const { min, max } = this.$store.getters['suburbs/statistics/chartDataSoldMedianPriceGlobalMinMax']
    const range = max - min
    const offset = 0.3
    const chartMin = min - range * offset > 0 ? min - range * offset : 0
    const chartMax = max + range * offset
    this.min = chartMin
    this.max = chartMax
    ...
    this.chart = new Chart(this.$refs['chart'], {
      ...
      options: {
        scales: {
          yAxes: [{ ticks: { display: false, min: chartMin, max: chartMax }, gridLines: { drawTicks: false, display: false, drawBorder: false } }],
